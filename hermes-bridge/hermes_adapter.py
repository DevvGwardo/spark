...
# --------------------------------------------------------------------------
# Brain MCP for pooled cross-session caching
# --------------------------------------------------------------------------

# Brain gateway configuration (from openclaw config)
...
class _CircuitBreaker:
    ...
    def is_available(self) -> bool:
        ...
        if self.state == "open":
            import time as _time
            if self.last_failure_time and (_time.monotonic() - self.last_failure_time) >= self.recovery_timeout:
...
# Circuit breaker for brain gateway
...
def _retry_brain_call(func, *args, retries: int = 2, backoff: float = 0.5, **kwargs):
    ...
    for attempt in range(retries + 1):
        if not _brain_circuit.is_available():
            print(f"[hermes-adapter] brain circuit is {'open' if _brain_circuit.get_state() == 'open' else 'half-open'}, skipping call", flush=True)
            return None
        try:
            ...
            print(f"[hermes-adapter] brain call attempt {attempt + 1} failed: {e}", flush=True)
            _brain_circuit.record_failure()
        if attempt < retries:
...
def _get_brain_token() -> Optional[str]:
    """Load Bearer token from openclaw.json gateway config."""
    ...
    try:
        ...
        pass
    # Fallback: env var
    _BRAIN_GATEWAY_TOKEN = os.environ.get("BRAIN_GATEWAY_TOKEN", "")
    return _BRAIN_GATEWAY_TOKEN


# Brain gateway retry configuration
_BRAIN_HTTP_TIMEOUT = float(os.environ.get("HERMES_BRAIN_HTTP_TIMEOUT", "5.0"))


def _brain_http_call(method: str, path: str, json_body: Optional[dict] = None) -> Optional[dict]:
    """Make a synchronous HTTP call to the brain gateway. Returns None on failure."""
    token = _get_brain_token()
    if not token:
        print("[hermes-adapter] brain gateway token not available", flush=True)
        return None
    try:
        headers = {
        ...
        with httpx.Client(timeout=_BRAIN_HTTP_TIMEOUT) as client:
            if method == "GET":
                ...
                resp = client.post(f"{_BRAIN_GATEWAY_URL}{path}", headers=headers, json=json_body)
            else:
                return None
            if resp.status_code in (200, 201):
                return resp.json() if resp.content else {}
            if resp.status_code == 401:
                print(f"[hermes-adapter] brain gateway auth failed (401) — token may be expired", flush=True)
            elif resp.status_code == 404:
                print(f"[hermes-adapter] brain gateway endpoint not found: {path}", flush=True)
            return None
    except httpx.TimeoutException:
        print(f"[hermes-adapter] brain gateway timeout ({_BRAIN_HTTP_TIMEOUT}s) for {method} {path}", flush=True)
        return None
    except Exception as e:
        print(f"[hermes-adapter] brain HTTP call failed for {method} {path}: {e}", flush=True)
        return None


def _brain_safe_set(key: str, value: str, ttl: int = 300) -> bool:
    """Store in brain cache with TTL and circuit-breaker retry."""
    result = _retry_brain_call(_brain_http_call, "POST", "/state/set", {"key": key, "value": value, "ttl": ttl})
    return result is not None


def _brain_safe_get(key: str) -> Optional[str]:
    """Retrieve from brain cache. Returns None on miss or brain unavailable."""
    result = _retry_brain_call(_brain_http_call, "GET", f"/state/get?key={quote(key, safe='')}")
    if result and "value" in result:
        return result["value"]
    return None


def _brain_safe_delete(key: str) -> bool:
    """Delete from brain cache. Graceful no-op if brain unavailable."""
    result = _retry_brain_call(_brain_http_call, "POST", "/state/delete", {"key": key})
    return result is not None
